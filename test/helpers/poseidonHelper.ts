...
export async function deployPoseidons(signer: any, poseidonSizeParams: number[], isLog: boolean = true) {
  ...
  const deployPoseidon = async (params: number, isLog: boolean) => {
    ...
    const PoseidonElements = new ethers.ContractFactory(abi, code, signer);
...
export async function deployPoseidonFacade(signer: any, isLog: boolean = true) {
